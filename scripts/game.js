import { changeBalance, setBalanceField, shuffle } from "./functions.js";
...
document.querySelector(".wrapper").style.background =
  "url(../png/back_" +
  localStorage.getItem("chosen_snake") +
  ".png) center/100% 100%";
...
function checkValidMove() {
  for (let i = 1; i < snakeCoords.length; i++) {
    if (
      snakeCoords[0][0] == snakeCoords[i][0] &&
  ...
  if (
    snakeCoords[0][0] < 0 ||
    snakeCoords[0][0] >= rowAmount ||
    snakeCoords[0][1] < 0 ||
    snakeCoords[0][1] >= colAmount
  ) {
    return false;
  }
...
function generateItem(type) {
  for (let i of shuffle(
    Array.from({ length: colAmount * rowAmount }, (v, i) => i)
  )) {
    if (
      cells[i].classList.contains("snake") ||
      cells[i].classList.contains("currency") ||
      cells[i].classList.contains("food_" + level)
    ) {
      continue;
    }
...
function gameOver(result) {
  if (result) {
    ...
    if (level != 6) {
      let levels = Array.from(
        localStorage.getItem("levels_snake").split(","),
        (el) => Number(el)
      );
      levels.push(level + 1);
    ...
    gameOverTab.querySelector(".outcome").innerHTML =
      "Level " + level + " is complited!";
  ...
  localStorage.setItem(
    "fruit_snake",
    Number(localStorage.getItem("fruit_snake")) + score
  );
}
